...
colnames(matrixTrain)[1201] <- "NACE26"
colnames(matrixTest)[1201] <- "NACE26"
table(matrixTrain$NACE26)
table(matrixTest$NACE26)
# load("matrixTrain.RData")
# load("matrixTest.RData")
#matrixTrain$NACE26 <- ifelse(matrixTrain$NACE26 == 2, 0, 1)
#matrixTest$NACE26 <- ifelse(matrixTest$NACE26 == 2, 0, 1)
# wrd <- read.csv("rf_200_words.csv")
# colnames(wrd) <- "words"
# words <- as.character(wrd$words)
# matrixTrain <- matrixTrain[, c("NACE26",words)]
# matrixTest <- matrixTest[, c("NACE26",words)]
...
table(matrixTrain$NACE26)/sum(table(matrixTrain$NACE26))
table(matrixTest$NACE26)/sum(table(matrixTest$NACE26))
t1 <- table(matrixTrain$NACE26)/nrow(matrixTrain)
t1
t2 <- table(matrixTest$NACE26)/nrow(matrixTest)
t2
quant <- round(t1[1],2)
quant
observTrain <- matrixTrain$NACE26
observTest <- matrixTest$NACE26


# ----------------------------------------------------------------
# Random Forest
matrixTrain$NACE26 <- as.factor(matrixTrain$NACE26)
matrixTest$NACE26 <- as.factor(matrixTest$NACE26)
rf <- randomForest(NACE26 ~ ., 
               # maxdepth = 3,
               importance=TRUE, 
               ntree = 500,
